              ...
              {Object.entries(grouped).map(([key, group]) => (
                <div key={key} id={`cat-${key}`} className="scroll-mt-24">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-5">
                    ...
                    {key === 'cache-ecran' && (
                      <Link
                        to="/cache-ecran-personnalise-sur-mesure"
                        className="group relative flex flex-col justify-between p-5 rounded-2xl bg-gradient-to-br from-accent/10 via-card to-secondary/40 border-2 border-dashed border-accent/40 hover:border-accent hover:shadow-premium-lg transition-all duration-300 hover:-translate-y-1 min-h-[260px] overflow-hidden"
                      >
                        <div className="absolute -top-10 -right-10 w-32 h-32 bg-accent/10 rounded-full blur-2xl pointer-events-none" />
                        <div className="relative">
                          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-accent text-accent-foreground text-[10px] font-bold uppercase tracking-wide">
                            <Sparkles className="w-3 h-3" /> Sur mesure
                          </div>
                          <h4 className="mt-3 font-display font-bold text-base md:text-lg text-foreground leading-tight">
                            Cache écran 100% personnalisé
                          </h4>
                          <p className="mt-2 text-xs md:text-sm text-muted-foreground leading-relaxed">
                            Création unique selon votre univers, vos goûts, votre personnalité.
                          </p>
                        </div>
                        <div className="relative mt-4 flex items-center justify-between">
                          <span className="text-base font-extrabold text-foreground">19,90 €</span>
